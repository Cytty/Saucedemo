...
@Feature("Тесты на сортировку товаров на сайте https://www.saucedemo.com/ для проблемного пользователя")
@Story("Тесты на сортировку товаров на сайте https://www.saucedemo.com/ для проблемного пользователя")
@Epic("Проблемный пользователь")
public class SortingProblemUserTests extends BaseTest {
    static final Logger logger = LoggerFactory.getLogger(SortingProblemUserTests.class);
    ...
    void priceToLowStandardUserTest() {
        logger.info("Тест на функцию сортировки ToLow");
        new InventoryPage( driver)
    ...
    static void endOfSortingTests() {
        logger.info("Тесты на функцию сортировки для проблемного пользователя завершены");
    }
}
